...
import { useChronalogSession } from "../../utils/auth/hooks"
...
function AdminLayoutContent({ children }: AdminLayoutProps) {
    ...
    async function fetchHomeUrl() {
      try {
        ...
        if (data.success && data.info?.homeUrl) {
          // Trim whitespace and ensure we have the exact value
  ...
  return (
    <div className="flex min-h-screen flex-col bg-zinc-50 font-sans dark:bg-black">
      <div className="sticky top-0 z-10 border-b border-zinc-200 bg-white/80 backdrop-blur-sm dark:border-zinc-800 dark:bg-black/80">
        <div className="flex items-center justify-between px-6 py-4">
          <div className="inline-flex flex-col">
            <h1 className="text-2xl font-bold text-black dark:text-zinc-50" style={{ fontFamily: "'Cormorant', serif" }}>
              Chronalog
            </h1>
            <svg
            ...
            {session && (
              <>
                {(() => {
                  ...
                  // Check if it's an external URL (case-insensitive check)
                  const lowerUrl = normalizedUrl.toLowerCase()
                  const isExternal = lowerUrl.startsWith("http://") || 
                                     lowerUrl.startsWith("https://") || 
                                     normalizedUrl.startsWith("//")
                  
                  if (isExternal) {
                    // Ensure protocol is present for protocol-relative URLs
      ...
      <footer className="border-t border-zinc-200 bg-white px-6 py-4 dark:border-zinc-800 dark:bg-black">
        <div className="flex items-center justify-center gap-4 text-sm text-zinc-600 dark:text-zinc-400">
          <p>
            © 2026 Chronalog. All rights reserved. Built by{" "}
            <a
